...
export default function Characters({ session }: {session: Session | "guest"}) {
  const { charList, addChar, onRemoveChar, resetChars } = useCharacters(session);
  ...
  return (
    <>
            {session === "guest" && <Alert className="mb-4" color="info">
        <span>
      ...
      <section>
        ...
        <p
          ...
          onClick={resetChars}
        >
          Click here to add the default characters
        </p>
      </section>
          ...
          <div className="mb-5">
            ...
            >
              ...
              <span className="text-sm text-gray-300">
                (It is recommended to start with something like 'a man...')
              </span>
            </Label>
            <textarea
              className="mt-1 bg-gray-700 border placeholder-gray-400 text-gray-100 text-sm rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block w-full p-2.5"
